...
def create_institution(
    institution: InstitutionPost, db: Session = Depends(get_db)
):
    institution_data = institution.dict()

    institution_data["user_id"] = 1 #we need to make this user id the session user id
...
def update_institution(
    institution: InstitutionUpdate, db: Session = Depends(get_db)
):
  try:
    institution_dict = institution.dict()
    
    db_institution = db.query(Institution).filter(Institution.name==institution.name).first() #need to edit to check for session user institution instead
